...
def load_data():
    """精確對應『庫存』工作表"""
    try:
        df = conn.read(worksheet="庫存", ttl="0s")
        ...
        try:
            ...
            st.error(f"❌ 讀取試算表失敗，請確認 Google Sheet 中有『庫存』工作表。細節：{e_inner}")
            st.stop()
...
def append_to_log_sheet(connection, possible_sheet_names, new_logs_df):
    ...
    for sheet_name in possible_sheet_names:
        try:
            try:
                df_existing = connection.read(worksheet=sheet_name, ttl="0s")
...
def get_log_sheet_data(connection, possible_sheet_names):
    for sheet_name in possible_sheet_names:
        try:
            df = connection.read(worksheet=sheet_name, ttl="0s")
...
if menu == "💊 多項藥品領用登記":
    ...
    if df_inventory.empty:
        st.info("目前庫存無任何資料，請先連線至 Google Sheets 或新增資料。")
    else:
        # 建立顯示格式：英文 (中文) - 批號:XXX
        df_inventory["display_name"] = (
            df_inventory["藥品名稱(英文)"].fillna("").astype(str)
            + " ("
            + df_inventory["中文名稱"].fillna("").astype(str)
            + ") - 批號:"
            + df_inventory["批號"].fillna("無").astype(str)
        )
    ...
    with tab_inbound:
        if df_inventory.empty:
            ...
            if st.session_state.inbound_stage == "input":
                ...
                if selected_med:
                    ...
                    with col_p2:
                        new_batch = st.text_input(
                            "🏷️ 新藥品批號：",
                            value=str(row_info.get("批號", "")) if pd.notna(row_info.get("批號")) else "",
                            key="new_batch",
                        )
                        new_expiry = st.date_input("⏳ 新有效期限：", value=datetime.now().date(), key="new_expiry")

                    vendor_remark = st.text_input(
                        "🏢 廠商/採購備註：", placeholder="例如：衛福部撥發 / 某某藥局採購", key="vendor_remark"
                    )
                ...
                st.write(
                    f"📦 **進貨數量：** `{i_data['purchase_qty']}` （原有庫存：{i_data['old_qty']} ➔ **進貨後總庫存：{i_data['old_qty'] + i_data['purchase_qty']}**）"
                )
                st.write(
                    f"🏷️ **新批號：** `{i_data['new_batch']}` | ⏳ **新有效期限：** `{i_data['new_expiry']}`"
                )
                ...
                with col_i1:
                    if st.button("✅ 確認進貨並同步更新雲端", type="primary"):
                        ...
                        try:
                            ...
                            st.session_state.inbound_stage = "input"
                            st.success(
                                f"🎉 進貨完成！`{i_data['selected_med']}` 庫存已由 {i_data['old_qty']} 增加至 {new_qty}。"
                            )
                            st.rerun()
                        except Exception as e:
                            st.error(f"❌ 庫存更新失敗：{e}")
        ...
        if st.button("✨ 建立新藥品並寫入雲端", type="primary"):
            if not new_eng_name.strip():
                ...
                for col in df_save.columns:
                    ...
                    elif "用途" in col_s or "適應" in col_s:
                ...
                try:
                    conn.update(worksheet="庫存", data=df_save)
                    st.cache_data.clear()
                    st.success(f"🎉 成功建立新藥品『{new_eng_name} ({new_cht_name})』！已同步寫入『庫存』分頁。")
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ 寫入雲端失敗：{e}")
...
# -----------------------------------------------------------------------------
    ...
    st.header("📦 當前藥品庫存總覽")
    display_df = df_inventory.drop(columns=["display_name"], errors="ignore").copy()
...
# -----------------------------------------------------------------------------
    ...
    for idx, row in df_inventory.iterrows():
        ...
        expired_writeoff = 0
        if pd.notna(expiry) and expiry != "" and expiry.lower() != "nan" and expiry.lower() != "none":
            try:
